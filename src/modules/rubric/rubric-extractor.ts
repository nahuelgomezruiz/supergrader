...
import { RubricResult, RubricItem, StructuredRubric } from '../../types/index';
...
export class RubricExtractor {
  ...
  extractRubric(): RubricResult {
    ...
    const scoreBox = root.querySelector<HTMLInputElement>(
      'input[name="score"], input[type="number"][placeholder*="score" i], input[type="number"][id*="score" i]'
    );
   ...
   */
  private extractFromDOM(domItems: HTMLElement[]): StructuredRubric {
    ...
    const rubricStyle: 'CHECKBOX' | 'RADIO' | 'MIXED' = 
      hasRadio && hasCheckbox ? 'MIXED' : hasRadio ? 'RADIO' : 'CHECKBOX';
   ...
   */
  private extractRadioGroup(element: HTMLElement, index: number): RubricItem | null {
   ...
   */
  private extractCheckboxItem(element: HTMLElement, index: number): RubricItem | null {
   ...
   */
  private async expandGroupIfNeeded(group: Element): Promise<void> {
   ...
   * Collapse group
   */
  private async collapseGroup(group: Element): Promise<void> {
