...
import { WindowWithExtensions, RubricItem } from '../types/index';
...
async function showRadioDiag(): Promise<void> {
...
async function initializeModularAPI() {
  try {
    ...
    const windowExt = window as WindowWithExtensions;
...
function showEnhancedRubricData() {
  ...
  if (rubricResult.type === 'structured') {
    ...
    rubricResult.items.forEach((item: RubricItem, index: number) => {
...
initializeModularAPI().then(success => {
  if (success) {
    ...
    (window as any).showEnhancedRubricData = showEnhancedRubricData;
    (window as any).showRadioDiag = showRadioDiag;
