...
class GradescopeAPI {
        ...
        while (downloadQueue.length > 0 || activeDownloads.size > 0 || failedDownloads.length > 0) {
            ...
            while (activeDownloads.size < MAX_CONCURRENT_DOWNLOADS && downloadQueue.length > 0) {
                ...
                if (!fileName || !this.shouldProcessFile(fileName, result.metadata))
                    continue;
                const downloadPromise = this.downloadFileWithRetry(fileInfo, fileName, submissionId, RETRY_ATTEMPTS, RETRY_DELAY_MS).then(content => {
                    if (content) {
     ...
     */
    async downloadFileWithRetry(fileInfo, fileName, submissionId, maxAttempts, retryDelay) {
        let lastError = null;
    ...
    if (domItems.length) {
        ...
        items.forEach(item => {
            ...
            if (item.element) {
                ...
                if (hasInput) {
                    const inputEmoji = hasInput.checked ? '✅' : '⬜';
                    console.log(`    ${inputEmoji} Input state: ${hasInput.checked}`);
                }
            }
        });
        return { type: 'structured', items, rubricStyle };
...
function initializeAPITesting() {
                ...
                rubricResult.items.forEach((item, index) => {
                    ...
                    if (item.description && item.description.includes('Available options:')) {
                        ...
                        const selected = item.element ?
                            (window.GradescopeAPI?.isRubricItemSelected?.(item.element) || false) : false;
    ...
    window.supergrader.showRadioButtons = () => {
        ...
        if (typeof getRubric === 'function') {
            const rubricResult = getRubric();
            if (rubricResult?.type === 'structured') {
                const radioItems = rubricResult.items.filter((item) => item.itemType === 'RADIO' || (item.description && item.description.includes('Available options:')));
                ...
                radioItems.forEach((item, index) => {
                    ...
                    if (item.description) {
                        ...
                        if (optionLines.length > 0) {
                            console.log('Available Options:');
                            optionLines.forEach((option, optIndex) => {
...
window.showRadioDiag = async () => {
    ...
    const expandIfNeeded = async (group) => {
