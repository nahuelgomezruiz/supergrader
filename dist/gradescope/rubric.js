 ...
 */
async function getCachedRubric(cacheKey) {
 ...
 */
async function setCachedRubric(cacheKey, data) {
 ...
 */
async function fetchFreshRubricData(courseId, assignmentId) {
    ...
    for (let i = 0; i < questions.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (question) => {
            try {
                ...
                const rubricStyle = rubricItems.select_one ? "RADIO" : "CHECKBOX";
                // Build question data
                const questionData = {
                    name: question.name,
 ...
 */
async function fetchQuestionRubricItems(courseId, questionId) {
